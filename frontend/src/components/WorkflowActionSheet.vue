...
<!-- <template>
	<div v-if="actions.length > 0" :class="[
		props.view === 'form'
			? 'px-4 pt-4 pb-4 standalone:pb-safe-bottom sm:w-96 bg-white sticky bottom-0 w-full drop-shadow-xl z-40 border-t rounded-t-lg'
			: 'flex w-full flex-row items-center justify-between gap-3 sticky bottom-0 border-t z-[100] p-4',
	]">
		<Button v-if="props.view === 'form' || actions.length > 2" @click="showTransitions"
			class="w-full rounded py-5 text-base disabled:bg-gray-700 disabled:text-white" variant="solid">
			<template #prefix>
				<FeatherIcon name="chevron-up" class="w-4" />
			</template>
			Actions
		...
		<template v-else>
			<Button v-for="action in actions" :key="action.text" class="w-full py-5" :variant="action.variant"
				:theme="action.theme" @click="applyWorkflow({ workflowAction: action.text })">
				<template #prefix v-if="action.featherIcon">
					<FeatherIcon :name="action.featherIcon" class="w-4" />
				</template>
				{{ action.text }}
	...
	<ion-action-sheet :buttons="actions" :is-open="showActionSheet"
		@didDismiss="applyWorkflow({ event: $event })"></ion-action-sheet>
	<div v-if="showRemarksDialog" class="dialog-overlay">
		<div class="dialog-content">
			<h3 class="dialog-title">Enter Remarks</h3>
			<textarea v-model="remarks" placeholder="Enter your remarks here..." class="dialog-textarea"></textarea>
...
const applyWorkflow = async ({ event = "", workflowAction = "" }) => {
	const action = workflowAction || event.detail.data?.action;
	if (action) {
		if (action.toLowerCase() === "reject" || action.toLowerCase() === "review") {
...
const submitRemarks = async () => {
	if (remarks.value) {
		...
		if (remarks.value) {
			...
			await props.workflow.applyWorkflow(props.doc, "Review", remarks.value); // Default workflow action
		}
		emit("workflow-applied");
		remarks.value = "";
		closeDialog();
...
<template>
	...
	<div v-if="showRemarksDialog.status" class="dialog-overlay">
		<div class="dialog-content">
			<h3 class="dialog-title">Enter Remarks</h3>
			<textarea v-model="remarks" placeholder="Enter your remarks here..." class="dialog-textarea"></textarea>
			<div class="dialog-actions">
...
let showActionSheet = ref(false);
let actions = ref([]);
let showRemarksDialog = ref({ status: false, action: '', remarks: '' }); // State to control the remarks dialog visibility
let remarks = ref(""); // State for the remarks input
...
const getTransitions = async () => {
	...
	actions.value = transitions.map((transition) => {
		...
		if (actionLabel.includes("reject") || actionLabel.includes("cancel")) {
			...
			variant = "solid";
			icon = "check";
		} else if (actionLabel.includes("review")) {
			theme = "yellow";
			variant = "solid";
			icon = "edit";
		}
...
const applyWorkflow = async ({ event = "", workflowAction = "" }) => {
	const action = workflowAction || event.detail.data?.action;
	if (action) {
		if (["reject", "review"].includes(action.toLowerCase())) {
			// Open the dialog for remarks if action is 'reject' or 'review'
			showRemarksDialog.value.status = true;
			showRemarksDialog.value.action = action;
			return; // Prevent further execution until remarks are submitted
		}
...
const closeDialog = () => {
	remarks.value = "";
	showRemarksDialog.value.status = false; // Hide the remarks dialog
	showRemarksDialog.value.action = '';
};
